...
study = optuna.create_study(
    direction="maximize",
    storage=storage_url,
    study_name="opt-emissoes",
    load_if_exists=True,
)  ### study for F1 metric, thats why maximize
study.optimize(
    lambda trial: objective(trial, images_dir, labels_dir, model, data_yaml),
    n_trials=50,
)
# %%
from ultralytics import YOLO
...
def on_train_epoch_end(trainer):
    ...
    current_recall = trainer.metrics.get(
        "metrics/recall(B)", 0.0
    )  # Obtenha o recall atual dos resultados de validação
