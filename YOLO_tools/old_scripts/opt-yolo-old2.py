...
def train_yolo(space):
    ...
    with open(
        r"D:\Judson_projetos\Yolo_trainer\YOLO_tools\training\params.json", "w"
    ) as file:
...
ray.init(
    runtime_env={"working_dir": folder_project},
    include_dashboard=True,
    _temp_dir=f"{folder_project}/ray_sessions",
)
...
trainable_with_resources = tune.with_resources(
    train_yolo, {"cpu": NUM_THREADS, "gpu": gpu_per_trial or 0}
)
