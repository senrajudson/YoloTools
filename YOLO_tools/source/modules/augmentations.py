            ...
            for file in os.listdir(pasta_imagens):
                ...
                transform = A.Compose(
                    [
                        A.OneOf(
                            [
                                ...
                                A.GaussianBlur(blur_limit=(2, 5), p=1),
                                A.MotionBlur(blur_limit=5, p=1),
                                A.RandomBrightnessContrast(
                                    brightness_limit=0.3, contrast_limit=0.3, p=1
                                ),
                                A.ISONoise(
                                    color_shift=(0.01, 0.05), intensity=(0.1, 0.2), p=1
                                ),
                                A.SaltAndPepper(p=1),
                                A.RandomFog(alpha_coef=0.05, p=1),
                                A.RandomSnow(brightness_coeff=1.5, p=1),
                                A.RandomRain(
                                    drop_length=5, drop_width=2, blur_value=5, p=1
                                ),
                                A.RandomSunFlare(
                                    flare_roi=(0.5, 0.5, 1.0, 1.0),
                                    src_radius=30,
                                    p=1,
                                    method="physics_based",
                                ),
                                # # Transformações no nível espacial
                                A.BBoxSafeRandomCrop(erosion_rate=0.3, p=1),
                                ...
                                A.VerticalFlip(p=1),
                                A.SafeRotate(limit=20, border_mode=0, p=1),
                                A.GridDropout(ratio=0.1, p=1),
                    ...
                    bbox_params=A.BboxParams(
                        format="yolo", label_fields=["class_labels"], min_visibility=0.7
                    ),
                )
                ...
                for i in range(int(n_aug)):
                    ...
                    save_yolo_annotations(
                        new_txt_path, transformed_bboxes, transformed_class_labels
                    )
                    ...
                    if folder != None:
                        ...
                        for file in os.listdir(pasta):
                            ...
                            for i in range(int(n_aug)):
                                ...
                                aug_image_path = "/".join(
                                    [pasta, f"{filename}_{i}.png"]
                                )
                                os.makedirs(
                                    os.path.dirname(aug_image_path), exist_ok=True
                                )
                                cv2.imwrite(aug_image_path, transformed_image)
            ...
            for ann in img_annotations:
                ...
                transform = A.Compose(
                    [
                        ...
                        A.RandomBrightnessContrast(
                            brightness_limit=0.05, contrast_limit=0.05, p=odd
                        ),
                        A.ISONoise(
                            color_shift=(0.005, 0.02), intensity=(0.05, 0.1), p=odd
                        ),
                        A.SaltAndPepper(p=odd),
                        ...
                        A.SmallestMaxSize(
                            max_size=None,
                            max_size_hw=(int(img_height * 0.75), int(img_width * 0.75)),
                            p=odd,
                        ),
                        A.VerticalFlip(p=odd),
                        A.SafeRotate(limit=10, border_mode=0, p=odd),
                    ...
                    bbox_params=A.BboxParams(
                        format="coco", label_fields=["class_labels"], min_visibility=0.4
                    ),
                )
