...
def model_Yolo_classification():
    ...
    def receber_imagem_da_fila_teste():
        if imagens_de_exemplo:
            return imagens_de_exemplo.pop(
                0
            )  # Remover e retornar a primeira imagem da lista
        else:
    ...
    while True:
        ...
        for result in resultado:
            ...
            if float(class_prob_value) < 0.9:
                print(
                    f"A imagem {imagem} é classificada como '{class_names}' com probabilidade {class_prob_value} está abaixo do padrão."
                )
                continue

            print(
                f"A imagem {imagem} é classificada como '{class_names}' com probabilidade {class_prob_value}."
            )
